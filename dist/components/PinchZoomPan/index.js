...
const getOffsets = (zoom, {
  ...
  return {
    xOffset: R.clamp(0, 10000, xOffset),
    yOffset: R.clamp(0, 10000, yOffset)
  };
};
...
export default function PinchZoomPan({
  ...
  const bind = useGesture({
    onDrag: ({
      movement,
      memo = [x.getValue(), y.getValue(), scale.getValue() + zoom.getValue()]
    }) => {
      const [x, y, zoom] = memo;
      ...
      const {
        xOffset,
        yOffset
      } = getOffsets(zoom, initialDims, heightZoomFactor);
    ...
    onPinch: ({
      ...
      const newZoom = {
        zoom: R.clamp(lowerBound, upperBound, normalizedNewZoom)
      };
